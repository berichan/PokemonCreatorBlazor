...
// Adapted from https://github.com/kwsch/PKHeX/blob/master/PKHeX.Drawing.PokeSprite/Util/SpriteUtil.cs
namespace PokemonCreator.PokeSprite
{
    public static class SpriteName
    {
        ...
        private const char FormSeperator = '-';
        ...
        public static string GetResourceStringSprite(int species, int form, int gender, uint formarg, int generation = PKX.Generation, bool shiny = false)
        {
            ...
            if (form != 0)
            {
                sb.Append(FormSeperator).Append(form);
            ...
            if (species == (int)Species.Alcremie)
            {
                if (form == 0)
                    sb.Append(FormSeperator).Append(form);
                sb.Append(FormSeperator).Append(formarg);
            }
