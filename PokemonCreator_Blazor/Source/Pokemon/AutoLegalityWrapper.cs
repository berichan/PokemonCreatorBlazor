...
namespace PokemonCreator
{
    public static class AutoLegalityWrapper
    {
        ...
        {
            if (!IsFixedOT(new LegalityAnalysis(pkm).EncounterOriginal, pkm))
                return false;
        ...
        public static ITrainerInfo GetTrainerInfoType(Type t)
        {
            if (t == typeof(PK8))
                return TrainerSettings.GetSavedTrainerData(GameVersion.SWSH, 8);
            if (t == typeof(PB8))
            ...
            throw new ArgumentException("Type does not have a recognized trainer fetch.", t.Name);
        }
